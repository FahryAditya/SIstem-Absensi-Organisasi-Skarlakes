...
export default function QrCodeClient({ baseUrl, initialItems }: QrCodeClientProps) {
  ...
  const isVercelPreview = typeof window !== 'undefined' && window.location.hostname.includes('-git-') && window.location.hostname.includes('.vercel.app')
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="card p-5">
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-[380px_1fr] gap-6 items-start">
              <div className="rounded-xl border border-royal-200 bg-royal-900 p-4 relative aspect-square w-full max-w-[360px]">
                <Image 
                  src={qrImage} 
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-xs font-bold text-royal-400 uppercase tracking-widest">QR Aktif</div>
                  <div className="text-2xl font-black text-white mt-1">OSIS & MPK</div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="rounded-lg bg-cream-50/80 border border-royal-200 p-3"><div className="text-xs text-royal-400">Valid Dari</div><div className="text-sm font-bold">{formatDateTime(activeQr.valid_from)}</div></div>
                  <div className="rounded-lg bg-cream-50/80 border border-royal-200 p-3"><div className="text-xs text-royal-400">Valid Sampai</div><div className="text-sm font-bold">{formatDateTime(activeQr.valid_until)}</div></div>
                  <div className="rounded-lg bg-cream-50/80 border border-royal-200 p-3"><div className="text-xs text-royal-400">Scan Sah</div><div className="text-sm font-bold">{activeQr._count.antrian} data</div></div>
                  <div className="rounded-lg bg-cream-50/80 border border-royal-200 p-3"><div className="text-xs text-royal-400">Dibuat Oleh</div><div className="text-sm font-bold">{activeQr.creator?.nama || '-'}</div></div>
                </div>
                {expiredSoon && (
                  <div className="rounded-xl bg-yellow-bright-500/10 border border-royal-200 p-3 text-sm text-yellow-bright-400 flex gap-2">
      ...
      <div className="space-y-5">
        ...
        <div className="card overflow-hidden">
          <div className="px-5 py-4 border-b border-royal-200 flex items-center justify-between gap-3">
            <div className="text-sm font-bold text-white">Riwayat QR</div>
            {items.length > 0 && (
              <button
                onClick={() => {
                ...
                className={deleteMode ? 'btn-secondary btn-sm text-royal-300' : 'btn-secondary btn-sm text-red-400'}
              >
          ...
          <div className="divide-y divide-slate-100">
            {items.length === 0 ? (
              <div className="p-5 text-sm text-royal-400">Belum ada riwayat.</div>
            ) : items.map((item) => (
              <div key={item.id} className={`p-4 ${deleteMode && selectedDeleteId === item.id ? 'bg-red-500/50' : ''}`}>
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`w-7 h-7 rounded-full border flex items-center justify-center transition ${
                          selectedDeleteId === item.id
                            ...
                            : 'bg-royal-900 border-royal-300 text-transparent hover:border-red-300 hover:bg-red-500/10'
                        }`}
                        title="Pilih QR ini"
                      >
                    ...
                    <span className={item.aktif ? 'badge bg-green-500/10 text-green-400 border border-royal-200' : 'badge bg-white/10 text-royal-400 border border-royal-200'}>{item.aktif ? 'Aktif' : 'Nonaktif'}</span>
                    <span className="text-xs text-royal-400">#{item.id}</span>
                  </div>
                  {!deleteMode && (
                ...
                <div className="text-xs text-royal-400 mt-2">Berlaku: {formatDateTime(item.valid_from)} - {formatDateTime(item.valid_until)}</div>
              </div>
            ))}
          </div>
          {deleteMode && (
            <div className="px-5 py-4 border-t border-royal-200 bg-cream-50/80 flex items-center justify-end gap-2">
      ...
      <Modal open={createModal} title="Buat QR Code Baru" onClose={() => setCreateModal(false)} size="md"
        ...
        <div className="space-y-4">
          <div className="text-sm text-royal-300 leading-relaxed mb-4">
