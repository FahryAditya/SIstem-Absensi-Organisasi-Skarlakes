        ...
        <div className="card p-5">
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-[380px_1fr] gap-6 items-start">
              <div className="rounded-xl border border-white/10 bg-deep-navy p-4 relative aspect-square w-full max-w-[360px]">
                <Image 
                  src={qrImage} 
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">QR Aktif</div>
          ...
          <div className="divide-y divide-slate-100">
            {items.length === 0 ? (
              ...
              <div key={item.id} className={`p-4 ${deleteMode && selectedDeleteId === item.id ? 'bg-red-500/10/50' : ''}`}>
                ...
                <div className="text-xs text-slate-400 mt-2">Berlaku: {formatDateTime(item.valid_from)} - {formatDateTime(item.valid_until)}</div>
              </div>
            ))}
          </div>
          {deleteMode && (
            <div className="px-5 py-4 border-t border-white/10 bg-white/5 flex items-center justify-end gap-2">
