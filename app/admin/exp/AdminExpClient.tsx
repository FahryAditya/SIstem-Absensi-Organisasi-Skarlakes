          ...
          <div className="bg-[#0f1117] border border-white/10 rounded-2xl p-6">
            ...
            <div className="space-y-5">
              <div className="relative" ref={dropdownContainerRef}>
                <label className="block text-sm font-medium text-royal-400 mb-2">Pilih Anggota</label>
                
                <button
                  type="button"
                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  disabled={loadingMembers}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-royal-500 text-left flex items-center justify-between transition-all hover:bg-white/10 hover:border-white/20 disabled:opacity-50"
                >
                  {selectedMember ? (
                    <div className="flex items-center justify-between w-full pr-2">
                      <div className="flex items-center gap-2.5">
                        ...
                        {selectedMember.kelas && (
                          <span className="text-xs px-2 py-0.5 rounded bg-white/10 text-royal-300 font-medium">
                            {selectedMember.kelas}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs bg-white/20 text-royal-400 px-2 py-0.5 rounded font-bold">
                          Lv.{selectedMember.level}
                        </span>
                        <span className="text-xs text-royal-400">{selectedMember.xp} EXP</span>
                      </div>
                    </div>
                  ) : (
                  ...
                  <ChevronDown className={`w-4 h-4 text-royal-400 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                ...
                {isDropdownOpen && (
                  <div className="absolute left-0 right-0 mt-2 bg-[#161922] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden backdrop-blur-xl animate-in fade-in duration-200">
                    <div className="p-2 border-b border-white/5 relative">
                      <Search className="w-4 h-4 text-royal-400 absolute left-3 top-1/2 -translate-y-1/2" />
                      <input
                        type="text"
                    ...
                    <ul className="max-h-60 overflow-y-auto py-1 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                      {filteredMembers.length === 0 ? (
                        <li className="px-4 py-6 text-sm text-royal-400 text-center">
                        ...
                        filteredMembers.map(m => {
                          ...
                          return (
                            <li key={m.id}>
                              ...
                              >
                                <div className="flex items-center gap-2.5">
                                  <span className={`font-medium ${isSelected ? 'text-royal-400' : 'text-white'}`}>
                                    {m.nama}
                                  </span>
                                  {m.kelas && (
                                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-royal-400 font-medium">
                                      {m.kelas}
                                    </span>
                                  )}
                                </div>
                                <div className="flex items-center gap-3">
                                  <div className="flex items-center gap-1.5">
                                    <span className="text-xs bg-white/10 text-royal-400 px-1.5 py-0.5 rounded font-semibold">
                                      Lv.{m.level}
                                    </span>
                                    <span className="text-xs text-royal-400">{m.xp} EXP</span>
                                  </div>
                                  {isSelected && <Check className="w-4 h-4 text-royal-400" />}
                                </div>
                              </button>
                            </li>
                          )
                ...
                {loadingMembers && !isDropdownOpen && (
                  <p className="text-xs text-royal-400 mt-2 animate-pulse">Memuat anggota...</p>
                )}
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-royal-400 mb-2">Jenis</label>
                  <div className="flex p-1 bg-white/5 border border-white/10 rounded-xl">
                    <button
                      className={`flex-1 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition ${isAdd ? 'bg-emerald-500/20 text-emerald-400' : 'text-royal-400 hover:text-white'}`}
                      onClick={() => setIsAdd(true)}
                    >
                      <Plus className="w-4 h-4" /> Tambah
                    </button>
                    <button
                      className={`flex-1 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition ${!isAdd ? 'bg-red-500/20 text-red-400' : 'text-royal-400 hover:text-white'}`}
                ...
                <div>
                  <label className="block text-sm font-medium text-royal-400 mb-2">Jumlah EXP</label>
                  <input
                    type="number"
                    min="1"
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-royal-500"
                    value={amount}
              ...
              <div>
                <label className="block text-sm font-medium text-royal-400 mb-2">Alasan / Keterangan</label>
                <textarea
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-royal-500 resize-none"
                  rows={3}
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl bg-gradient-to-r from-royal-500 to-royal-600 text-white font-bold hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2"
              >
        ...
        <div className="lg:col-span-1">
          <div className="bg-[#0f1117] border border-white/10 rounded-2xl p-6 h-full">
            ...
            <div className="space-y-4">
              {loadingLogs ? (
                <p className="text-sm text-royal-400 text-center py-10">Memuat riwayat...</p>
              ) : logs.length === 0 ? (
                <p className="text-sm text-royal-400 text-center py-10">Belum ada riwayat update manual.</p>
              ) : (
                logs.map(log => (
                  <div key={log.id} className="p-3 bg-white/5 border border-white/10 rounded-xl">
                    ...
                    <p className="text-xs text-royal-400 mt-1 line-clamp-2">{log.alasan}</p>
                  </div>
                ))
              )}
            </div>
