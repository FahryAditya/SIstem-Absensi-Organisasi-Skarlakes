...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = osisMpkSchema.safeParse(body)
    ...
    if (!org || (org.tipe !== 'osis' && org.tipe !== 'mpk')) {
      return NextResponse.json({ error: 'Organisasi tidak valid' }, { status: 400 })
    }

    // 2. Check duplicate
    const existing = await prisma.registrationOsisMpk.findFirst({
      where: {
        email_gmail,
        organization_id,
    ...
    // 3. Save registration
    const registration = await prisma.registrationOsisMpk.create({
      data: {
        ...parsed.data,
        status: 'CALON'
      }
    ...
    return NextResponse.json({ 
      success: true, 
      data: registration,
      message: 'Pendaftaran berhasil dikirim. Tunggu pengumuman penerimaan melalui email.'
    })
  } catch (error: any) {
    console.error('[REGISTRATION OSIS-MPK ERROR]', error)
