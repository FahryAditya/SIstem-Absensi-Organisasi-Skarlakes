      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <div className="xl:col-span-2 space-y-4">
          ...
          <div className="card overflow-hidden">
            ...
            <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/60 grid grid-cols-1 sm:grid-cols-3 gap-2">
              <select value={hasilFilter} onChange={(e) => setHasilFilter(e.target.value)} className="input py-2">
                <option value="">Semua Hasil</option>
                <option value="LOLOS">Lolos</option>
                <option value="TIDAK_LOLOS">Tidak Lolos</option>
              </select>
              <input value={kelasDraft} onChange={(e) => setKelasDraft(e.target.value)} className="input py-2" placeholder="Filter kelas, contoh X IPA 1" />
            </div>
            ...
            {loading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead><tr className="bg-slate-50 border-b border-slate-200"><th className="th">No</th><th className="th">Kandidat</th><th className="th">Sesi</th><th className="th">Status</th><th className="th">Hasil</th><th className="th">Persentase</th><th className="th">Override</th><th className="th"></th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {queue.map((q) => (
                      <tr key={q.id} className="hover:bg-slate-50">
                        ...
                        <td className="td"><span className="badge bg-white border border-slate-200 text-slate-600">OSIS & MPK</span></td>
                        ...
                        <td className="td">{q.hasil_wawancara?.override_at ? <div className="text-xs text-amber-700"><div className="font-bold">Override</div><div>{q.hasil_wawancara.overrider?.nama || 'Administrator'}</div></div> : <span className="text-xs text-slate-400">-</span>}</td>
                        <td className="td">
                          {(q as any).sesiStatus === 'ACTIVE' ? (
                            <div className="flex gap-1 justify-end">
                              {q.status === 'MENUNGGU' && <button onClick={() => setQueueStatus(q.id, 'WAWANCARA')} className="btn-secondary btn-sm"><Clock className="w-3.5 h-3.5" />Panggil</button>}
                              <button onClick={() => openResult(q)} className="btn-primary btn-sm"><SquarePen className="w-3.5 h-3.5" />Nilai</button>
