    ...
    <div className="space-y-5">
      ...
      <div className="page-header">
        ...
        <div className="flex gap-2">
          <button onClick={() => setClearConfirmOpen(true)} className="btn-secondary text-red-400 border-royal-800/30 hover:bg-red-500/10 flex items-center gap-2">
      ...
      <div className="card overflow-hidden">
              ...
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i} className="border-b border-royal-800/30">
                    {Array.from({ length: 7 }).map((_, j) => (
                      <td key={j} className="td"><div className="h-4 bg-royal-200 rounded animate-pulse" style={{ width: `${50 + Math.random()*40}%` }} /></td>
                    ))}
                  </tr>
                ))
              ) : logs.length === 0 ? (
                <tr><td colSpan={7} className="td py-16 text-center text-royal-400">
                ...
                logs.map(log => (
                  <tr key={log.id} className="tr">
                    ...
                    <td className="td">
                      <span className="font-mono text-xs text-royal-400 bg-cream-50/10 px-1.5 py-0.5 rounded">{log.tabel}</span>
                    </td>
                    <td className="td">
                      <p className="text-sm text-royal-200 leading-snug max-w-xs truncate" title={log.deskripsi}>{log.deskripsi}</p>
                    </td>
                    <td className="td">
                      <span className="text-[10px] font-mono text-royal-400">{log.ip_address || '-'}</span>
                    </td>
                    <td className="td">
                      {(log.data_lama || log.data_baru) && (
                        <button onClick={() => setDetailLog(log)} className="btn-icon text-royal-400 hover:bg-royal-600/10">
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-royal-800/30 bg-cream-50/50">
            <span className="text-xs text-royal-400">{total} total log — Halaman {page} dari {totalPages}</span>
        ...
        {detailLog && (
          <div className="space-y-4">
            ...
            <div className="form-group">
              ...
              <div className="p-3 bg-cream-50/5 rounded-lg text-sm text-royal-200 border border-royal-800/30">{detailLog.deskripsi}</div>
            </div>
            ...
            {detailLog.data_lama && (
              <div className="form-group">
                ...
                <pre className="p-3 bg-red-500/10 border border-royal-800/30 rounded-lg text-xs text-red-400 overflow-auto max-h-40 font-mono">
            ...
            {detailLog.data_baru && (
              <div className="form-group">
                ...
                <pre className="p-3 bg-green-500/10 border border-royal-800/30 rounded-lg text-xs text-green-400 overflow-auto max-h-40 font-mono">
