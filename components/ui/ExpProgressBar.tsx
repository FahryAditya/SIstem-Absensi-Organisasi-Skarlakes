...
export function ExpProgressBar({ exp, showLabels = true }: ExpProgressBarProps) {
  ...
  const gradientClass = {
    ...
    4: 'from-indigo-500 to-indigo-600 shadow-indigo-200/50',
    ...
    <div className="w-full flex flex-col gap-1.5">
      {showLabels && (
        <div className="flex justify-between items-baseline text-xs font-semibold">
          <span className="text-[#052659] flex items-center gap-1">
            <span>Progress</span>
            <span className="text-[10px] font-mono bg-slate-100 border border-slate-200/60 text-slate-500 px-1.5 py-0.5 rounded">
              {Math.round(persen)}%
            </span>
          </span>
          <span className="font-mono text-[11px] text-slate-500">
            {isMaster ? (
              ...
              <>
                <strong className="text-slate-700">{expSaatIni}</strong>
      ...
      {/* Progress Track */}
      <div className="w-full h-2.5 bg-slate-100 rounded-full border border-slate-200/40 p-[1.5px] overflow-hidden shadow-inner">
        <div
