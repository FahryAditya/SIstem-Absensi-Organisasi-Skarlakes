...
export default function Topbar({
  ...
  const [orgOpen, setOrgOpen] = useState(false); // org switcher
  const [loggingOut, setLoggingOut] = useState(false);
  const [switching, setSwitching] = useState<number | null>(null);
  const [myOrgs, setMyOrgs] = useState<{ id: number; nama: string; slug: string }[]>([]);

  useEffect(() => {
    // Fetch organization names for the switcher
  ...
  async function handleSwitchOrg(orgId: number) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error('Gagal mengalihkan organisasi');
      }
    } catch (e) {
  ...
  return (
    <header className="sticky top-0 z-30 shrink-0 bg-deep-navy border-b border-white/10 h-14 flex items-center px-4 lg:px-6 gap-4">
      ...
      <div className="flex-1 flex items-center gap-3 min-w-0">
        <h1 className="text-sm font-bold text-white truncate hidden md:block">{pageTitle}</h1>
        ...
        {myOrgs.length > 0 && (
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-xs font-bold text-white"
            >
            ...
            {orgOpen && (
              <>
                ...
                <div className="absolute left-0 top-full mt-2 w-56 bg-deep-navy/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/10 py-1.5 z-50 animate-in fade-in zoom-in duration-150">
                  ...
                  <div className="max-h-[300px] overflow-y-auto custom-scrollbar">
                    {myOrgs.map(org => (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2 text-xs font-bold transition-colors ${org.id === user.activeOrgId ? 'text-persian-blue bg-persian-blue/10' : 'text-white hover:bg-white/5'}`}
                      >
                        <span className="truncate">{org.nama}</span>
                        {org.id === user.activeOrgId && <Check className="w-3.5 h-3.5" />}
                        {switching === org.id && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                      </button>
                    ))}
                  </div>
                </div>
